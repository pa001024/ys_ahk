...
impl<'a> Cooker<'a> {
    pub fn new(console: &'a Term, cfg: Config, hwnd: HWND) -> Self {
        ...
        Self {
            ...
            okimg: {
                let img = image::load_from_memory(include_bytes!("../ok.png"))
                    .unwrap()
                    .to_luma32f();
                img
            },
    ...
    pub fn run(&mut self) {
        ...
        if debug {
            ...
            self.log(format!("State: {:?}", self.state));
            // self.AutoReply();
            // self.Publish();
            for i in 1..10 {
                self.SendEmo(i);
            ...
            // self.PlayActions(self.cfg.actions.on_enter.clone());
            // self.next();
            return;
        }

        let state_bar: ProgressBar = self.bars.add(ProgressBar::new_spinner());
        let step_bar: ProgressBar = self.bars.add(ProgressBar::new_spinner());
        state_bar.enable_steady_tick(Duration::from_millis(200));
        step_bar.enable_steady_tick(Duration::from_millis(200));
        while self.ctl.isForeground() {
            self.state = self.GetState();
            state_bar.set_message(format!("{}: {:?}", style("State:").blue(), self.state));
            step_bar.set_message(format!("{}: {:?}", style("Step:").blue(), self.step));
            self.next();
            sleep(200);
        }
        state_bar.finish();
    ...
    fn log(&self, msg: String) {
        ...
        self.bars.println(format!("{timestamp} {msg}"));
    }
            ...
            GameState::Main => {
                if self.check_time.elapsed().as_secs() > 5 {
                    ...
                    if count >= 3 {
                        ...
                        bar.set_message(format!("{}", style("UID池已满 等待中...").red()));
    ...
    pub fn AutoReply(&mut self) {
        // self.log(format!("{}", style("AutoReply").green()));
        let bar = self.bars.add(ProgressBar::new_spinner());
        bar.enable_steady_tick(Duration::from_millis(200));
        bar.set_message(format!(
            "{}: {:?}",
            style("AutoReply").green(),
            self.reply_state
        ));
