...
cfg_checkmap_teleport = config.getint("cook", "checkmap_teleport")
...
cfg_level_pattern = config.get("cook", "level_pattern")
cfg_level_list_delay = config.getfloat("cook", "level_list_delay")
...
class GameControl:
    ...
    def GetState(self):
        if self.CheckColor(924, 818, "ECE5D8") and self.CheckColor(675, 818, "313131"):  # 下方复活 真: 死亡界面
            return "dead"
        if self.CheckColor(876, 633, "4A5366") and self.CheckColor(671, 628, "313131"):  # 旧版本提示 真: 提示界面
            return "esc"
    ...
    def AutoF2(self):
        with console.status("[red]AutoF2"):
            while self.isForeground() and not self.CheckColor(333, 333, "1C1C22|FFFFFF"):
                ...
                screen = ImageGrab.grab()
                for i in range(6):
                    if self.CheckColorS(screen, 1352, 158 + 104 * i, "3.3.3."):
                        self.Click(1349, 197 + 104 * i)
                        time.sleep(0.01)
                pg.press("esc")
                time.sleep(0.1)
    ...
    replyGroups = [
        ...
        [r"[那哪][三3]?个|说说看|怎么打|材料|几只|什么|多久|啥|^[\?？]|。。。|\.\.\.", "就是枫丹湖中垂柳右边的地方传奇，每天刷新的~~ 2分钟差不多打完了~", "idle"],
